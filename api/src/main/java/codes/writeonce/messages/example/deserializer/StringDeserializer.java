...
import codes.writeonce.messages.example.StringInterner;
...
public abstract class StringDeserializer implements Deserializer<String> {
    ...
    @Override
    public int consume(@Nonnull byte[] source, int start, int end) {

        switch (state) {
            case 0:
                start = intDeserializer.consume(source, start, end);
                if (start == -1) {
                    context.push(intDeserializer);
                    state = 3;
                    return start;
            ...
            case 3:
                ...
                if (value < 0) {
                    size = -value;
                    state = 2;
                    return startLatin1(source, start, end);
                } else if (value > 0) {
                    size = value;
                    if (chars == null || chars.length < value) {
                        chars = new char[value];
                    ...
                    position = 0;
                    hash = 0;
                    return appendUtf16(source, start, end);
                } else {
                    this.value = "";
                    state = 0;
                    return start;
